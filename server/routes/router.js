...
router.post("/createNewContact",userMiddleware.isAdmin, (req, res, next) => {
  const { to, name, message,timestamp } = req.body;
  db.query(
    "insert into chat (message_id,phone,timestamp,message,name,sent,status) values (?,?,?,'',?,-1,?) ON DUPLICATE KEY UPDATE status=?",
    [Math.floor(Math.random() * Math.pow(10, 62)),to,timestamp,name,"recieved","recieved"],
    (err, result) => {
      if (err) {
        console.error(err)
      }
